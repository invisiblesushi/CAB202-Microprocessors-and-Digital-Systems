...
int main() {
    ...
    while (1) {
        ...
        // TODO
        //curr_time = get_system_time(0);
        if (curr_time < get_system_time(0)){
          curr_time = get_system_time(0);
        }
...
void init_hardware(void) {
    ...
    // TODO
    TCCR1B &= ~(1 << WGM12);
    // Set the prescaler for TIMER1 so that the clock overflows every ~8.3 seconds 8.3880
    // TODO
    TCCR1B |= (1<<CS12);
    TCCR1B &= ~((1<<CS11));
    TCCR1B &= ~((1<<CS10));
...
double get_system_time(unsigned int timer_count) {
    ...
    // TODO
    double timer = TCNT1 / (FREQUENCY/PRESCALER);
    return timer;
}
