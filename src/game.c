...
void process(){
  ...
  if (lives > 0) {
    ...
    if ((get_system_ms() - (((int)get_system_time() % 60) * 1000 )) > 500 && (get_system_ms() - (((int)get_system_time() % 60) * 1000 )) < 600){
    ...
    if (!((missile.y > 10) && (missile.y < SCREEN_HEIGHT - 2) && (missile.x > 2) && (missile.x < SCREEN_WIDTH - 2))){
      missile_air = 0;
    }
}
  //Game over
...
void spawn_player(){
  ...
  //If spawn overlap
  if(player.x < alien.x + 10 && player.x > alien.x - 10 && player.y < alien.y + 10 && player.y > alien.y - 10){
    spawn_player();
  }
}
void spawn_alien(){
...
void missile_init(){
  if (missile_air == 0){
    ...
    if (gun.bitmap == gun_up_bitmap){
      gun_pos = 'u';
      init_sprite(&missile, player.x + (player.width/3) + 1, player.y - 3, 4, 2, missile_img);
    }
    else if(gun.bitmap == gun_down_bitmap){
      gun_pos = 'd';
      init_sprite(&missile, player.x + (player.width/3) + 1, player.y + player.height + 1, 4, 2, missile_img);
    }
    else if(gun.bitmap == gun_left_bitmap){
      gun_pos = 'l';
      init_sprite(&missile, player.x - 3, player.y + (player.height/2) -1, 2,4, missile_img);
    }
    else if(gun.bitmap == gun_right_bitmap){
      gun_pos = 'r';
      init_sprite(&missile, player.x + player.width +1, player.y + (player.height/2) -1, 2,4, missile_img);
    }
    missile_attack();
    missile_air =1;
    char buffer_user[20];
