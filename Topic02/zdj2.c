...
bool collision(void) {
    ...
    int hero_top = round(sprite_y(hero)), hero_base = hero_top + HERO_HEIGHT - 1;
    int hero_left = round(sprite_x(hero)), hero_right = hero_left + HERO_WIDTH - 1;

    int zombie_top = round(sprite_y(zombie)), zombie_base = zombie_top + ZOMBIE_HEIGHT - 1;
    int zombie_left = round(sprite_x(zombie)), zombie_right = zombie_left + ZOMBIE_WIDTH - 1;

    if ( hero_base < zombie_top ) collided = false;
    else if ( hero_top > zombie_base ) collided = false;
    else if ( hero_right < zombie_left ) collided = false;
    else if ( hero_left > zombie_right ) collided = false;
